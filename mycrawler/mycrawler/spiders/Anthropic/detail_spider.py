...
class AnthropicNewsDetailSpider(scrapy.Spider):
    name = "anthropic-detail"
    start_urls = []
    
    # Custom settings cho output file routes theo source
    custom_settings = {
        'FEEDS': {
            'data/Anthropic/anthropic-detail.json': {
                'format': 'json',
                'encoding': 'utf-8',
                'store_empty': False,
                'overwrite': True,
    ...
    async def start(self):
        ...
        if hasattr(self, 'start_url') and self.start_url:
            ...
            self.logger.error("⚠️ Không có URL được cung cấp. Sử dụng: scrapy crawl anthropic-detail -a start_url=<URL>")
            return
    ...
    def parse(self, response):
        """Parse detail page của Anthropic news article"""
    ...
    def _extract_title(self, response):
        ...
        if title:
            title = title.strip()
            # Loại bỏ " | Anthropic" nếu có
            title = re.sub(r'\s*\|\s*Anthropic.*$', '', title, flags=re.IGNORECASE)
            if len(title) > 0:
                return title
    ...
    def _extract_content(self, response):
        ...
        # Anthropic news thường sử dụng article tag hoặc main content area
        main_content_xpaths = [
            ...
            "//div[contains(@class, 'article')]//text()[normalize-space()]",
        ]
